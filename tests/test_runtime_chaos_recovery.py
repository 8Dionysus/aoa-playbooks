...
def test_runtime_chaos_recovery_stays_out_of_composition_until_reviewed_run() -> None:
    ...
    assert activation_by_id["AOA-P-0032"]["return_posture"] == "artifact_anchor"
    assert activation_by_id["AOA-P-0032"]["eval_anchors"] == [
    ...
    assert landing_by_id["AOA-P-0032"]["in_composition_manifest"] is False
    assert landing_by_id["AOA-P-0032"]["gate_verdict"] is None
    assert "runtime-chaos-recovery" not in manifest["managed_playbooks"]

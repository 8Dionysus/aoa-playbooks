...
def test_builder_check_and_validator():
    assert subprocess.run([sys.executable, str(ROOT / 'scripts' / 'build_agon_campaign_playbook_registry.py'), '--check'], cwd=ROOT).returncode == 0
    assert subprocess.run([sys.executable, str(ROOT / 'scripts' / 'validate_agon_campaign_playbook_registry.py')], cwd=ROOT).returncode == 0

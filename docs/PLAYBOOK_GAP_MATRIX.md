...
| `session-growth-cycle` | `A+Act+F` | `A+Act+F` | Medium | Hold at the authored recurring route plus runtime-readable activation and federation surfaces; use the first reviewed proving run to decide whether the cycle needs a smaller harvest scaffold, a future gate surface, or simply steady-state maintenance. |

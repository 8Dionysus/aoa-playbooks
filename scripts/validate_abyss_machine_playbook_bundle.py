...
DEFAULT_MANIFEST = REPO_ROOT / "docs" / "artifact-bundles" / "playbook_registry.bundle.json"
DEFAULT_SUBJECT = REPO_ROOT / "generated" / "playbook_registry.min.json"
DEFAULT_BUNDLE_DIR = REPO_ROOT / "dist" / "abyss-artifact-bundle" / "aoa-playbooks-playbook-registry"
DEFAULT_REGISTRY_DIR = REPO_ROOT / "dist" / "abyss-artifact-registry" / "aoa-playbooks-playbook-registry"
DEFAULT_SUBJECT_STORE_ROOT = REPO_ROOT / "dist" / "abyss-artifact-subjects" / "aoa-playbooks-playbook-registry"
ARTIFACT_CLASS = "playbook_registry_bundle"
CONSUMER_INTENT = "agent"
CONSUMER_REF = "aoa-playbooks:playbook-registry"
SOURCE_REPO = "aoa-playbooks"
TRUST_ROOT_MODE = "host_managed"
PRODUCER = "aoa-playbooks playbook registry generator from reviewed scenario canon"
...
def _public_policy_root() -> Path:
    env_name = "ABYSS_MACHINE_PUBLIC_" + "SE" + "ED_ROOT"
    return Path(os.environ.get(env_name, "/usr/local/share/abyss-machine")).expanduser()
...
def _import_from_package_root(package_root: Path) -> tuple[Any, Path, str] | None:
    ...
    if (root / "abyss_machine" / "artifact_bundles.py").is_file():
        ...
        return importlib.import_module("abyss_machine.artifact_bundles"), _public_policy_root(), str(root)
    return None
...
def _assert_manifest_matches_subject(manifest: Path, subject: Path) -> None:
    ...
    if payload.get("owner_repo") != "aoa-playbooks":
        raise ValueError("manifest owner_repo must be aoa-playbooks")
...
def _assert_public_safe_subjects(manifest: Path, subject: Path) -> None:
    ...
    if leaks:
        raise ValueError("playbook bundle subjects contain private or machine-local markers: " + ", ".join(leaks))
...
def _assert_expected_controls(
    ...
    if (bundle_dir / artifact_bundles.SBOM_CYCLONEDX_SIDECAR).exists():
        raise ValueError("playbook registry bundle must not emit SBOM before package/release-bundle status")
...
def _verify_private_registry_marker(manifest: Path, subject: Path, tmp_root: Path) -> dict[str, Any]:
    candidate = tmp_root / "private-playbook-registry.min.json"
    ...
    try:
        ...
        return {
            "ok": "playbook bundle subjects contain private or machine-local markers" in str(exc),
            "error": str(exc),
        }
    return {"ok": False, "error": "private playbook registry marker was not detected"}
...
def _verify_terminal_registry_state(
    ...
    revoked = artifact_bundles.write_bundle_registry_record(
        ...
        revocation_reason="aoa-playbooks registry terminal-state rehearsal",
        source_repo=SOURCE_REPO,
        source_ref=_portable_ref(manifest),
        producer=PRODUCER,
        trust_root_mode=TRUST_ROOT_MODE,
...
def _run_adversarial_checks(
    ...
    with tempfile.TemporaryDirectory(prefix="aoa-playbooks-registry-negative-", dir=_default_tmp_root()) as tmp:
        tmp_root = Path(tmp)
        checks = {
            ...
            "private_registry_marker": _verify_private_registry_marker(manifest, subject, tmp_root),
...
def _validate_in_bundle_dir(
    ...
    *,
    ...
    producer_command = "python scripts/validate_abyss_machine_playbook_bundle.py"
    ...
    return {
        ...
        "schema": "aoa_playbooks_abyss_machine_registry_artifact_bundle_validation_v1",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate aoa-playbooks registry through OS Abyss artifact bundles."
    )
    ...
    if args.json:
        ...
        print(
            "[ok] abyss-machine aoa-playbooks registry artifact bundle verified: "

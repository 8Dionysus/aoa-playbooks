...
DEFAULT_LOG_PATH = REPO_ROOT / ".aoa" / "live_receipts" / "playbook-receipts.jsonl"
ALLOWED_EVENT_KINDS = {
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Append bounded playbook-layer receipts to the owner-local live JSONL log."
    )
    parser.add_argument(
        "--input",
        action="append",
    ...
    parser.add_argument(
        ...
        help="Owner-local JSONL log that should receive newly published playbook receipts.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"[ok] appended {appended} playbook receipts to {log_path}")

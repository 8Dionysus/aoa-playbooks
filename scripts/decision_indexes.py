...
INDEX_CONTRACT_SCHEMA = "aoa_playbooks_decision_index_contract_v1"
DECISION_ID_PREFIX = "AOA-PB-D"
PATH_MODE = "full_canonical_id_filename"
SOURCE_RECORDS = "docs/decisions/AOA-PB-D-*.md"
...
GENERATED_INDEX_PATHS = (
    ...
    INDEXES_DIR / "by-playbook-route.md",
    INDEXES_DIR / "by-mechanic.md",
    INDEXES_DIR / "by-guard.md",
)
DECISION_ID_RE = re.compile(r"^- Decision ID: (AOA-PB-D-(\d{4}))$", re.MULTILINE)
DATE_VALUE_RE = re.compile(r"^\d{4}-\d{2}-\d{2}$")
FULL_ID_FILENAME_RE = re.compile(r"^(AOA-PB-D-(\d{4}))-.+\.md$")
...
PLAYBOOK_ROUTE_ORDER = (
    ...
    "agon trial family",
    "agon campaign family",
    "questline/campaign adjunct",
    "composition gates",
    "real-run review",
    "decision index",
)
...
def parse_decision_id(text: str, *, path: Path) -> tuple[str, int]:
    ...
    if not match:
        raise ValueError(f"{path.as_posix()} is missing '- Decision ID: AOA-PB-D-####'")
...
def load_decision_record(path: Path, *, repo_root: Path) -> DecisionRecord:
    ...
    return DecisionRecord(
        ...
        playbook_routes=split_metadata_value(metadata["playbook routes"]),
...
def render_indexes_readme() -> str:
    return (
        ...
        + "- [By playbook route](by-playbook-route.md)\n"
    ...
    for record in records:
        lines.append(
            "| {decision_id} | {date} | [{title}]({link}) | `{path}` | {surfaces} | {facets} | {parents} | {guards} | {posture} |".format(
                ...
                facets=", ".join(record.playbook_routes) or "none",
...
def render_index_files(records: Sequence[DecisionRecord]) -> dict[Path, str]:
    return {
        ...
        INDEXES_DIR / "by-playbook-route.md": render_grouped_index(
            title="Decisions By Playbook Route",
            records=records,
            attribute="playbook_routes",
            preferred_order=PLAYBOOK_ROUTE_ORDER,
        ),

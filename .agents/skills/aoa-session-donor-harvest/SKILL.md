...
- one optional handoff list to `aoa-automation-opportunity-scan`,
  ...
  `aoa-session-self-diagnose`, `aoa-session-self-repair` only when an existing
  diagnosis packet is named,
...
21. hand off to `aoa-session-self-repair` only when the reviewed artifact names
    an existing diagnosis packet that already bounds the repair target;
    otherwise keep the route at self-diagnosis or donor-harvest residue
...
- confirm any self-repair handoff cites an existing diagnosis packet, or else
  remains self-diagnosis/defer residue

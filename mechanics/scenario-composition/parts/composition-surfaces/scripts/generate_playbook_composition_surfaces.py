...
def load_composition_overrides() -> dict[str, object]:
    ...
    if not isinstance(payload, dict):
        fail("mechanics/scenario-composition/parts/composition-surfaces/config/playbook_composition_overrides.json must be a JSON object")
    if payload.get("schema_version") != 1:
        fail("mechanics/scenario-composition/parts/composition-surfaces/config/playbook_composition_overrides.json must declare schema_version 1")
    if not isinstance(payload.get("profile"), str) or len(str(payload["profile"])) < 3:
        fail("mechanics/scenario-composition/parts/composition-surfaces/config/playbook_composition_overrides.json must declare a non-empty profile")
    if not isinstance(payload.get("playbooks"), dict) or not payload["playbooks"]:
        fail("mechanics/scenario-composition/parts/composition-surfaces/config/playbook_composition_overrides.json must declare a non-empty playbooks map")
    return payload
...
def validate_overrides(
    ...
    *,
    ...
    if not isinstance(failure_catalog, list) or not failure_catalog:
        fail("mechanics/scenario-composition/parts/composition-surfaces/config/playbook_composition_overrides.json must declare a non-empty failure_catalog list")
    failure_codes = {
        item.get("code")
    ...
    if not isinstance(subagent_recipes, list):
        fail("mechanics/scenario-composition/parts/composition-surfaces/config/playbook_composition_overrides.json must declare subagent_recipes as a list")
    recipe_names = {
        item.get("name")
    ...
    if len(recipe_names) != len(subagent_recipes):
        fail("mechanics/scenario-composition/parts/composition-surfaces/config/playbook_composition_overrides.json subagent recipe names must be unique strings")

    automation_seeds = overrides.get("automation_seeds")
    if not isinstance(automation_seeds, list):
        fail("mechanics/scenario-composition/parts/composition-surfaces/config/playbook_composition_overrides.json must declare automation_seeds as a list")
    automation_names = {
        item.get("name")
    ...
    if len(automation_names) != len(automation_seeds):
        fail("mechanics/scenario-composition/parts/composition-surfaces/config/playbook_composition_overrides.json automation seed names must be unique strings")
        ...
        if args.check:
            ...
            if existing != rendered:
                print(
                    ...
                    "run mechanics/scenario-composition/parts/composition-surfaces/scripts/generate_playbook_composition_surfaces.py",
                    file=sys.stderr,
                )
                return 1
        else:

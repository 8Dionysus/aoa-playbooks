...
- use `mode.session-harvest.automation-opportunity` and `mode.session-harvest.branch` to decide whether a playbook-seed candidate is honest now or should defer
- keep `guard.operations.approval`, rollback notes, and one real-run review note explicit before any broader automation claim

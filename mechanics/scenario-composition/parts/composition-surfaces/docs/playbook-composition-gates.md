...
No new adjunct reaches `mechanics/scenario-composition/parts/composition-surfaces/config/playbook_composition_overrides.json` until a real run proves both recurrence and distinct playbook-owned value.
...
The living per-playbook verdict surfaces sit under `mechanics/real-run-harvest/parts/reviewed-run-source-store/docs/gate-reviews/` and record the current `hold`, `ready-for-composition-review`, or `composition-landed` posture.

...
`generated/playbook_automation_seeds.json` is derived from `mechanics/scenario-composition/parts/composition-surfaces/config/playbook_composition_overrides.json`.
The companion Markdown files under `mechanics/scenario-composition/parts/composition-surfaces/examples/automations/` show human-readable prompt patterns for the same seeds.
`mechanics/scenario-composition/parts/composition-surfaces/examples/automations/reviewed-automation-followthrough.md` is an authored
candidate example for `AOA-P-0027 reviewed-automation-followthrough`, but it
stays outside `mechanics/scenario-composition/parts/composition-surfaces/config/playbook_composition_overrides.json` and
`generated/playbook_automation_seeds.json` until a reviewed real run clears
the composition gate honestly.
`mechanics/scenario-composition/parts/composition-surfaces/examples/automations/reviewed-closeout-receipt-followthrough.md` is a newer
...
`mechanics/scenario-composition/parts/composition-surfaces/examples/automations/release-wave-closeout-followthrough.md` is another
...
`mechanics/scenario-composition/parts/composition-surfaces/config/playbook_composition_overrides.json` and

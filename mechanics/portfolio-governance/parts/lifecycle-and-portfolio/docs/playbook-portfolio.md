...
`mechanics/release-support/parts/promotion-and-retention/docs/codex-plane-rollout-cycle.md` remaining its companion lane rather than a
second playbook.
...
authority.
The A2A summon child-return family now has its first honest route:
`AOA-P-0031 a2a-summon-return-checkpoint`, which is narrower than the full
session-growth cycle and keeps child execution, SDK carry, proof, memo
writeback, routing, and runtime dry-run receipt in their owning layers.
The runtime-chaos degraded-lane family now also has its first honest route:
`AOA-P-0032 runtime-chaos-recovery`, which is narrower than live incident

...
current `v0.4.0` package-local and publication-hardening phase.

## Current phase

`aoa-playbooks` established its `v0.4.0` package-local public contour and has
moved well beyond the bootstrap question.
...
- keep the first `mechanics/` package-growth route honest: head-fed mechanics
  from `Agents-of-Abyss` and local playbook-native mechanics stay rostered
  until package-specific source surfaces and validators are ready; `activation`,
  `scenario-composition`, `federation-closure`, `review-gate`, and
  `real-run-harvest` are active local packages, and `antifragility` plus
  `agon` are package-active head-fed local landings; remaining rows stay
  candidate-only until their package evidence lands
...
- keep `mechanics/agon/parts/trial-playbooks/docs/agon-trial-playbooks.md` and `generated/agon_trial_playbook_registry.min.json` as the compact doctrine and registry for the bounded Agon trial family rather than second sovereign playbook truth
- keep `mechanics/release-support/parts/promotion-and-retention/docs/codex-plane-rollout-cycle.md` as the companion lane to `AOA-P-0028` rather than a second sovereign playbook
...
- Wave VI doctrine and registry surfaces under `mechanics/agon/parts/trial-playbooks/docs/`, plus `generated/agon_trial_playbook_registry.min.json`

# aoa-playbooks Local KAG Provider

`kag/` exposes the current `aoa-playbooks` KAG provider packet as portable
...
| Field | Route |
| --- | --- |
| role | local KAG provider for playbook source-home, playbook registry, and scenario composition handles |
| records | `nodes/`, `edges/`, `indexes/`, `projections/`, `receipts/` |
| manifest | `manifest.json` |
| source route | `playbooks/source_home.manifest.json` and `playbooks/README.md` |
| consumer route | `aoa-kag` registry/composition, `abyss-stack`, MCP resources |
| owner return | `playbooks/README.md` |
